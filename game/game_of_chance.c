...
int main() {
  ...
  srand(time(0)); // Seed the randomizer witht the current time.
  ...
  while (choice != 7) {
    ...
    if ((choice < 1) || (choice > 7))
    ...
    else if (choice < 4) {       // Otherwise, choic was a game of some sort.
      if (choice != last_game) { // If the function ptr isn't set
        ...
        last_game = choice; // and set the last_game.
      }
      play_the_game();
    } else if (choice == 4)
      show_highscore();
...
// This function reads the player data ofr the current uid
...
int get_player_data() {
  ...
  if (read_bytes <
      sizeof(struct user)) // this means that the end of file was reached
    return -1;
  else
...
void register_new_player() {
    ...
    fd = opsen(DATAFILE, O_WRONLY|O_CREAT|O_APPEND, S_IRUSR|S_IWUSR);
    if(fd == -1)
...
void update_player_data(){
    ...
    read(fd, &read_uid, 4); // read the uid from the the first struct
    ...
    write(fd, &(player.highscore), 4); // update credits.
    write(fd, &(player.name), 100); // update credits.
    close(fd);
}

// this function will display the current high score append
...
void show_highscore(){
    ...
    while(read(fd, &entry, sizeof(struct user)) > 0) { // loop until end of file
        if(entry.highscore > top_score) { // if there is a higher score,
            ...
            stpcpy(top_name, entry.name); // and top_name to that username
        }
    }
    close(fd);
    if(top_score > player.highscore)
        printf("%s has the high score of %u credits!\n", top_name, player.highscore);
    printf("==========================================================\n\n");
}

// this function is used to input the player name, since
// scanf("%c", &whatever) will stop input at the first space.
void input_name() {
    ...
    while(input_char != '\n') { // loop until newline
        *name_ptr = input_char; // put the input char into name field.
        scanf("%c", &input_char); // get the next chars
        name_ptr++;
    }
    *name_ptr = 0; // terminate the string
}

// this function prints the 3 cards for the Find the Ace games
...
// Tihs function contains a loop to allow the current game to be
...
void play_the_game() {
    ...
    while(play_again) {
        ...
        if(player.current_game() != -1) {             // If the game plays without error and
            if(player.credits > player.highscore)     // a new hish score is set,
                player.highscore = player.credits;    // update the high score
...
int dealer_no_match() {
    ...
    if(match != -1) {
        ...
        printf("There were no matches! You wind %d credits!\n", wager);
        player.credits += wager;
    }
    return 0;
}
...
int find_the_ace() {
    ...
    pick = -1;
    while((pick < 1) || (pick > 3)) { // loop until a valid pick is made
